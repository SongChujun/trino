...
import io.trino.sql.planner.ConnectorExpressionRule;
...
import static io.trino.sql.planner.ConnectorExpressionPatterns.argument;
import static io.trino.sql.planner.ConnectorExpressionPatterns.argumentCount;
import static io.trino.sql.planner.ConnectorExpressionPatterns.arguments;
import static io.trino.sql.planner.ConnectorExpressionPatterns.call;
import static io.trino.sql.planner.ConnectorExpressionPatterns.expression;
import static io.trino.sql.planner.ConnectorExpressionPatterns.functionName;
import static io.trino.sql.planner.ConnectorExpressionPatterns.type;
        ...
        implements ConnectorExpressionRule<Call, ParameterizedExpression>
{
    ...
    public Optional<ParameterizedExpression> rewrite(Call call, Captures captures, RewriteContext<ParameterizedExpression> context)
    {
        ...
        parameters.addAll(value.get().getParameters());
        ...
        for (ConnectorExpression expression : expressions) {
            ...
            rewrittenValues.add(rewritten.get().getExpression());
            parameters.addAll(rewritten.get().getParameters());
        }

        List<String> values = rewrittenValues.build();
        verify(!values.isEmpty(), "Empty values");
        return Optional.of(new ParameterizedExpression(
                format("(%s) IN (%s)", value.get().getExpression(), Joiner.on(", ").join(values)),
                parameters.build()));
    }
}
